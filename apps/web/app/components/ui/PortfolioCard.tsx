...
export function PortfolioCard({
        ...
        <div className="flex flex-col md:flex-row h-full">
          ...
          <div className="md:w-1/2 p-8 flex flex-col justify-center space-y-4">
            ...
            <div className="flex gap-4 pt-4">
              <button className="flex-1 bg-surface-container-highest text-white py-2 rounded-xl text-xs font-bold hover:bg-indigo-500/20 transition-all border border-white/5">
                View Case Study
              </button>
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl bg-surface-container/60 backdrop-blur-xl border border-primary/5 transition-all hover:translate-y-[-4px] ${isLarge ? "md:col-span-8" : "md:col-span-4"}`}
    >
      ...
      <div className="p-6 space-y-4">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <h3
              className={`${isLarge ? "text-2xl" : "text-xl"} font-bold text-white group-hover:text-indigo-300 transition-colors`}
            >
              {title}
            </h3>
            <p className={`text-on-surface-variant ${isLarge ? "text-sm max-w-xl" : "text-xs"}`}>
              {description}
            </p>
          </div>
          {isLarge && (
        ...
        <div className={`flex flex-wrap gap-2 ${!isLarge ? "pt-2" : ""}`}>
          {liveStatus && (
            <span className="px-2 py-0.5 bg-surface-container-lowest text-[10px] font-mono text-tertiary rounded border border-tertiary/20">
              {liveStatus}
            </span>
          )}
          {tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 bg-surface-container-lowest text-[10px] font-mono text-on-surface-variant rounded border border-outline-variant/30"
            >
              {tag}
            </span>
          ))}
